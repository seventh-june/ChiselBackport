...
public class BlockSnakestone extends Block {
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entity, ItemStack stack) {
        ...
        if (rotateHead(world, x, y, z) != -1) return;
    ...
    public int[] getConnections(World world, int ox, int oy, int oz) {
        ...
        for (int direction = 0; direction < 6; direction++) {
            ...
            if (!match) {
                ...
                if (b1 != this || b2 != this) match = true;

                if (ox == px1 && oy == py1 && oz == pz1) match = true;

                if (ox == px2 && oy == py2 && oz == pz2) match = true;
            }

            if (match) res[index++] = direction;
        }

        if (index != 5) res[index] = -1;
    ...
    public boolean rotateBodyPart(World world, int x, int y, int z) {
        ...
        if (blockMeta != meta) world.setBlockMetadataWithNotify(x, y, z, meta, 3);
